...
void Database::getBooksByAuthor(List<Book>& results, const std::string& author) const noexcept {
    for (unsigned int i = 0; i < this->data.length(); ++i) {
        const Book& book = this->data[i];
...
void Database::getReferenceBooks(List<Book>& results) const noexcept {
    for (unsigned int i = 0; i < this->data.length(); ++i) {
        const Book& book = this->data[i];
        if (book.getCategory() == "Reference") {
            results.append(book);
        }
    }
}

unsigned short Database::countBooksByGenre(const std::string& genre) const noexcept {
